...
class WCCS_DB_Conditions extends WCCS_DB {
	...
	public function get_conditions( array $args = array() ) {
		...
		$conditions = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM  $this->table_name $where ORDER BY {$args['orderby']} {$args['order']} LIMIT %d,%d;", absint( $args['offset'] ), absint( $args['number'] ) ), $args['output'] );
