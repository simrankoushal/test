...
function twentynineteen_init() {

	register_sidebar(
		array(
			'name'          => __( 'Subject Post' ),
			'id'            => 'subject',
			'description'   => __( 'Show Subject Post.')
		)
	);

}
add_action( 'widgets_init', 'twentynineteen_init' );





add_filter( 'manage_edit-product_columns', 'product_date_column_after_name' );
function product_date_column_after_name( $product_columns ) {
	return array(
...
function add_product_col( $colname, $cptid ) {
     if ( $colname == 'product_order')
      	$units_sold = get_post_meta( $cptid, 'total_sales', true );
		echo $units_sold ;

}
add_action('manage_product_posts_custom_column', 'add_product_col', 10, 2);
